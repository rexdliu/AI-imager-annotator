...
def draw_canvas(image_path: str, rotation_angle: int = 0) -> Tuple[List[BBox], float, Optional[Image.Image], List[str]]:
    """Enhanced canvas with resizing, rotation, and coordinate readout.
    ...
    try:
        ...
        scale_factor = w_orig / display_w  # Scale factor for converting display coords to original coords
        ...
        r = int(stroke_color.lstrip('#')[0:2], 16)
        g = int(stroke_color.lstrip('#')[2:4], 16)
        b = int(stroke_color.lstrip('#')[4:6], 16)
