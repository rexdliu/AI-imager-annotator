...
class GeminiClient:
        ...
        def __init__(self, api_key: Optional[str] = None):
                ...
                if not self.api_key:
                        raise ValueError(
                                "Gemini API key must be provided either directly or as GEMINI_API_KEY environment variable")
                ...
                try:
                        ...
                        if image_path:
                                ...
                                with open(image_path, "rb") as image_file:
                                        ...
                                        parts.append({
                                                "mime_type": "image/jpeg",  # Assuming JPG, adjust as needed
                                                "data": image_data
                                        })
                        ...
                        if response.text:
                                ...
                                return "The response was blocked. Please try a different image or prompt."
        ...
        def get_qa_from_image(
                ...
                try:
                        ...
                        for i, qa_data in enumerate(qa_pairs_data):
                                ...
                                for field in ["task_type", "text_en", "text_local", "answer_en", "answer_local",
                                              "difficulty"]:
                                ...
                                # Handle case when task_type is "role_play" but it's using a different format
                                if qa_data.get("task_type") == "role play" or qa_data.get("task_type") == "roleplay":
                                        qa_data["task_type"] = "role_play"  # Normalize to the format we expect
                                ...
                                try:
                                        ...
                                        if "language_quality_score" in qa_data and not isinstance(
                                                qa_data["language_quality_score"], (int, float)):
                                                try:
                                                        qa_data["language_quality_score"] = float(
                                                                qa_data["language_quality_score"])
                                                except (ValueError, TypeError):
                                                        qa_data["language_quality_score"] = 3.0

                                        # Task type validation fix
                                        if "task_type" in qa_data and qa_data["task_type"] not in ["captioning", "vqa",
                                                                                                   "instruction",
                                                                                                   "role_play"]:
                                                qa_data["task_type"] = "vqa"  # Default to vqa for unknown task types
...
def get_system_prompt() -> str:
    ...
    For role_play tasks, create engaging scenarios where the user must interact with the image content.
    For example, "Imagine you are a food critic reviewing this dish. What would you say about it?"
